						...
						if(message.length > 0){
							...
							if('event' in message){
								...
								// when the track has changed we don't need a seek event
									...
									if (messages[index + 1]) {
										const nextMessage = JSON.parse(messages[index + 1]);
										if (nextMessage.event === 'start-file') {
											// if next message is start-file, dont destroy obrserveSocket. Happens when rapidly loading files
											return;
										}
									}

									observeSocket.destroy();
									reject(this.errorHandler.errorMessage(0, caller, [source, ...messages]));
								}
							}
						}
					});
